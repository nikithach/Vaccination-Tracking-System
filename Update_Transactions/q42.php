...
if(isset($_POST['Submit']))
{
 ...
 if($distributed_by=="Manufacturer"){
    $sql = "INSERT INTO manufacturers (manufacturer_name, vaccine_name, distributed_to, Count_distributed, date_manufactured) VALUES ('$manufacturer_name','$vaccine_name', '$state', '$no_of_vaccines','$date')";
    mysqli_query($conn,$sql);
    echo '<script>alert("Given amount of vaccines are distributed to a particular state.")</script>';
    header('Location:http://localhost/dbp/q421.php' ) ;

 }
 else {
  
   $sql = "INSERT INTO federal_data VALUES ('$vaccine_name','$state', '$no_of_vaccines','$manufacturer_name','$date')";
   mysqli_query($conn,$sql);
   echo '<script>alert("Given amount of vaccines are distributed to a particular state.")</script>';
...
<body>
  <div class="wrapper">
    ...
    <div class="middlenav">
      <h1>To distribute Vaccines to a state</h1>
    </div>
    ...
    <form method="POST" enctype="multipart/form-data" name="form">
      ...
      <br>
      <br>
        <label> <b> State: &nbsp;&nbsp;&nbsp;&emsp; &emsp;&emsp;&nbsp;&nbsp;</b></label> <input type="text" name="state" placeholder="Please enter your state" title="Please enter the state" pattern="[A-Za-z0-9@$&# ]{0,25}">
        <br>
        <br>
